...
@Component({
    ...
    <div class="user-root">
      <header class="header">
        ...
        <button mat-raised-button color="primary" (click)="createNewUser()">
          <mat-icon>person_add</mat-icon> {{ 'users.add' | translate }}
        </button>
      </header>
          ...
          <div class="new-user-form glass-panel" *ngIf="isCreating()">
            ...
            <div [formGroup]="userForm" class="form-row">
              ...
              <div class="form-actions">
                ...
                <button mat-stroked-button color="warn" (click)="cancelCreate()">CANCELAR</button>
              </div>
            </div>
          </div>
          <table mat-table [dataSource]="users()" class="desktop-table">
                  ...
                  <div class="edit-cell-group">
                    ...
                    <mat-form-field appearance="outline" subscriptSizing="dynamic">
                      ...
                      <mat-error *ngIf="userForm.get('code')?.hasError('required')"
                        >Requerido</mat-error
                      >
                      <mat-error *ngIf="userForm.get('code')?.hasError('pattern')"
                        >6 números</mat-error
                      >
                    </mat-form-field>
                  </div>
                ...
                <div class="actions">
                  <ng-container *ngIf="editingUserId() !== user.id; else editActions">
                    ...
                    <button
                      mat-icon-button
                      color="accent"
                      matTooltip="Gestionar Firmas"
                      (click)="manageSignatures(user)"
                    >
            ...
            <tr
              ...
              *matRowDef="let row; columns: displayedColumns"
              [class.editing-row]="editingUserId() === row.id"
            ></tr>
          </table>
          ...
          <div class="mobile-grid">
            <div
              *ngFor="let user of users()"
              ...
              [class.editing-card]="editingUserId() === user.id"
            >
  ...
  styles: [
    `
      ...
      .editing-card {
        border: 2px solid var(--primary-color);
        background: rgba(255, 122, 41, 0.05) !important;
      }
      .mobile-edit-form {
        display: flex;
        flex-direction: column;
      ...
      .editing-row {
        background: rgba(var(--accent-rgb, 0, 0, 0), 0.02);
      }
      .edit-cell-group {
        display: flex;
        flex-direction: column;
      ...
      .subscribe((confirmed) => {
        if (confirmed) {
          this.userService.updateUser(user.id, { isActive: false }).subscribe({
            ...
            error: (err: any) =>
              this.showError(`Error al desactivar usuario: ${err.message || err}`),
          });
        }
      });
  }
