...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatCardModule, MatIconModule, MatButtonModule, MatDialogModule, MatProgressSpinnerModule, MatTooltipModule],
  template: `
    <div class="gallery-root">
      <header class="header">
        ...
        <div class="header-actions">
          ...
          <button mat-stroked-button color="primary" (click)="fileInput.click()" [disabled]="!canCreate()" [matTooltip]="!canCreate() ? 'Ya posee una firma registrada. Deberá eliminarla para cargar una nueva.' : ''">
            <mat-icon>upload</mat-icon> CARGAR FIRMA
          </button>
          <button mat-raised-button color="primary" (click)="openCreator()" [disabled]="!canCreate()" [matTooltip]="!canCreate() ? 'Ya posee una firma registrada.' : ''">
      ...
      <div class="gallery-content">
        <div class="loader-overlay" *ngIf="isLoading()">
          ...
          <p>Cargando galería de firmas...</p>
        </div>
        ...
        <mat-card *ngFor="let sig of signatures()" class="sig-card glass-panel" [class.inactive]="!sig.isActive">
          ...
          <mat-card-content>
            <div class="sig-meta">
              ...
              <div class="m-item"><strong>Dimensiones:</strong> {{ sig.metadata.width }}x{{ sig.metadata.height }}</div>
              <div class="m-item"><strong>Formato:</strong> {{ sig.format }}</div>
            </div>
          </mat-card-content>
          <mat-card-actions align="end" *ngIf="isAdmin()">
            <button mat-button color="warn" *ngIf="sig.isActive" (click)="toggleStatus(sig)">DESACTIVAR</button>
            <button mat-button color="accent" *ngIf="!sig.isActive" (click)="toggleStatus(sig)">ACTIVAR</button>
  ...
  `,
  styles: [`
    .back-link { cursor: pointer; color: var(--primary-color); font-weight: 800; display: flex; align-items: center; gap: 4px; font-size: 11px; margin-bottom: 8px; text-transform: uppercase; }
    ...
    .gallery-content { position: relative; min-height: 400px; }
...
export class SignatureGalleryComponent implements OnInit {
  ...
  title = computed(() => this.isManagingOther() ? 'Gestión de Firma de Usuario' : 'Mis Firmas Registradas');
  subtitle = computed(() => this.isManagingOther() ? 'Configuración centralizada de firma para el colaborador seleccionado.' : 'Gestione sus firmas digitales para usar en los documentos del sistema');
  ...
  loadSignatures() {
    ...
    this.sigService.getSignaturesByUser(this.effectiveUserId()).subscribe((sigs: SignatureImage[]) => {
