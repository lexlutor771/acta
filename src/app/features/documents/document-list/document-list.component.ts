    ...
    <div class="list-container">
      <header class="list-header">
        <div>
          ...
          <p>{{ 'documents.listSubtitle' | translate }}</p>
        </div>
      ...
      <div class="filters-bar glass-panel">
        <mat-form-field appearance="outline" class="search-field">
          <mat-label>{{ 'documents.searchPlaceholder' | translate }}</mat-label>
          <mat-icon matPrefix>search</mat-icon>
          <input
            matInput
            [value]="columnFilters()['global'] || ''"
            (keyup)="applyFilter($event)"
            [placeholder]="'documents.searchExample' | translate"
          />
        </mat-form-field>
        ...
        <div class="filter-actions">
          <button
            mat-button
            (click)="toggleFilters()"
            [color]="showColumnFilters() ? 'primary' : ''"
          >
            <mat-icon>{{ showColumnFilters() ? 'filter_list_off' : 'filter_list' }}</mat-icon>
      ...
      <div class="table-wrapper glass-panel shadow-premium">
        <div class="loader-overlay" *ngIf="isLoading()">
          ...
          <p>{{ 'documents.syncing' | translate }}</p>
        </div>
        ...
        >
          ...
          <ng-container matColumnDef="progress">
            <th mat-header-cell *matHeaderCellDef class="col-progress">
              {{ 'documents.signers' | translate }}
            </th>
            <td mat-cell *matCellDef="let row" class="col-progress">
              <div class="progress-cell">
                <span class="progress-text"
                  >{{ getSignedCount(row) }}/{{ row.assignedSigners.length }}</span
                >
                <div class="progress-bar-bg">
                  <div
                    class="progress-bar-fill"
                    [style.width.%]="(getSignedCount(row) / row.assignedSigners.length) * 100"
                  ></div>
                </div>
              </div>
            </td>
          ...
          <ng-container matColumnDef="createdAt">
            ...
            <td mat-cell *matCellDef="let row" class="col-createdAt">
          ...
          <ng-container matColumnDef="actions">
            ...
            <td mat-cell *matCellDef="let row">
              <div class="action-buttons">
                <button
                  mat-icon-button
                  [routerLink]="['/documents', row.id, 'sign']"
                  [matTooltip]="'documents.signView' | translate"
                  color="primary"
                >
                ...
                <button
                  mat-icon-button
                  [routerLink]="['/documents', row.id, 'history']"
                  [matTooltip]="'documents.history' | translate"
                >
                ...
                <button
                  *ngIf="isAdmin()"
                  ...
                  color="accent"
                  [disabled]="row.status === 'PENDING' || isGeneratingPdf()"
                >
                  <mat-spinner
                    diameter="20"
                    *ngIf="isGeneratingPdf() && downloadingId() === row.id"
                  ></mat-spinner>
                  <mat-icon *ngIf="!(isGeneratingPdf() && downloadingId() === row.id)"
              ...
              <div class="filter-cell-wrapper" *ngIf="showColumnFilters() && col !== 'actions'">
                <mat-form-field appearance="outline" class="col-filter-field">
                  <input
                    ...
                    (keyup)="applyColumnFilter(col, $event)"
                    placeholder="Filtrar..."
                  />
                </mat-form-field>
              </div>
          ...
          <tr
            ...
            *matHeaderRowDef="displayedFilterColumns"
            [class.filter-row-hidden]="!showColumnFilters()"
          ></tr>
        ...
        <div class="mobile-grid" *ngIf="!isLoading()">
          <div
            class="doc-card glass-panel shadow-premium"
          ...
          >
            ...
            <div class="card-body">
              ...
              <div class="card-meta">
                <span>{{ 'documents.version' | translate }} {{ row.version }}</span>
                <span class="divider"></span>
                <span>{{ row.createdAt | dateFormat }}</span>
              </div>
              <div class="progress-section">
                <div class="progress-text">
                  <span>{{ 'documents.signatureProgress' | translate }}</span>
                  <span class="pct"
                    >{{
                      (getSignedCount(row) / row.assignedSigners.length) * 100 | number: '1.0-0'
                    }}%</span
                  >
                </div>
                <div class="progress-bar-bg">
                ...
                <div class="progress-text" style="opacity: 0.6; margin-top: 2px;">
                  <span
                    >{{ 'documents.signed' | translate }}: {{ getSignedCount(row) }}/{{
                      row.assignedSigners.length
                    }}</span
                  >
                </div>
              </div>
          ...
          <div *ngIf="dataSource.filteredData.length === 0" class="no-data-mobile">
  ...
  styles: [
    `
      ...
      @media (max-width: 768px) {
        ...
        .filters-bar .filter-actions button:first-child {
          height: 52px;
          border-radius: 12px;
          background: rgba(0, 0, 0, 0.03);
          opacity: 0.8;
        }
        .filters-bar .filter-actions button:last-child {
          width: 52px;
          height: 52px;
          border-radius: 12px;
      ...
      ::ng-deep .col-filter-field .mat-mdc-text-field-wrapper {
        height: 36px !important;
        border-radius: 8px !important;
        background: white !important;
      }
      ::ng-deep .col-filter-field .mat-mdc-form-field-infix {
        padding-top: 6px !important;
        padding-bottom: 6px !important;
        min-height: 36px !important;
      }
      ::ng-deep .col-filter-field .mat-mdc-form-field-input-control {
...
export class DocumentListComponent implements OnInit, AfterViewInit {
  ...
  constructor() {
    ...
    this.dataSource.filterPredicate = (data, filter) => {
      ...
      const matchDocumentCode = (data.documentCode || '')
        .toLowerCase()
        .includes(searchTerms.documentCode || '');
